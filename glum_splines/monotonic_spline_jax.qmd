---
title: "Building Monotonic Splines using JAX"
format: 
    gfm: 
        preview-mode: raw
keep-ipynb: True
jupyter: python3
execute:
  engine-path: "/Users/mm/Documents/Data Science/Blog Posts/venv11/bin/python"
knitr:
  opts_chunk: 
    echo: False
    cache: True
...
The constraint matrix combines these transformed coefficients so that the desired constraint is adhered to. We'll walk through how this works for decreasing trend. We know our transformed coefficients are strictly positive. So if we want our curve to always decrease, then each successive coefficient we multiply with our $X$ matrix needs to be strictly smaller than the previous coefficient. We can accomplish this by reparameterizing our coefficients once more. Lets set the first coefficient in our new coefficient vector to be the first transformed coefficient. The next coefficient now needs to be less than this first value. One way to do that is to subtract our $\tilde{\beta_2}$ value from $\tilde{\beta_1}$ and use this transformed coefficient. Since we know $\tilde{\beta_2}$ is positive (from using the exponential function) then we know that $\beta_1$ is strictly larger than $\tilde{\beta_1} - \tilde{\beta_2}$. We can repeat this logic the whole way down our vector of coefficients:
...
There is a phenomenal dataset of the first day of the Cherry Blossoms blooming in the Japanese Royal Gardens every year. I want to estimate the long-term trend of this date moving up over time due to Global Warming. There are natural, short-term fluctuations in this date based on the local climate in Japan so an accurate model of the year to year fluctuations will not be monotonically decreasing. But, for this post I'm only interested in the long-term trend which I'm going to assume only goes one way. We'll read in some data and build our model. 
...
We can fit a baseline GAM model using the `glum` library. 
...
It is absolutely amazing that we have a dataset of observations from before the year 1000 AD! There is a lot of variation in the short-term trends, which some could probably be smoothed out with a proper smooth penalty if we used proper GAM software like `mgcv` or `pygam`. Now we can get to our shape constrained version. First lets take a look at how we can 
...
cons_matrx = generate_constraint_matrix(base_model.coef_)
cons_matrix[:5, :5]
```

So we have our negative, lower triangle matrix that will apply our 2nd transformation of our coefficients. We now need a way to apply the two shape constraint steps given an unbounded coefficient vector.
...
```{python}
#coef_b = coefs[1:]
init_coefs = base_model.coef_
init_coefs = np.linspace(1, 0, 50+2)
coef_b = init_coefs
con_matr = generate_constraint_matrix(coef_b)
beta_exp = jnp.exp(coef_b)  # Ensure monotonicity
cumulative_sums = jnp.array(generate_constraint_matrix(coef_b))
constrained_coefs = jnp.matmul(cumulative_sums, beta_exp.T)
model_coefs = constrained_coefs
preds = jnp.dot(yearly_spline, model_coefs)
#np.mean(np.diff(constrained_coefs) <= 0)
#1.0
#np.mean(np.diff(preds) <= 0)
#0.77
```
...
result = minimize(
    fun=calc_loss,
    x0=np.array(init_coefs),  # SciPy requires NumPy
...
(ggplot(flower_df_clean.to_pandas(),aes(x='year')) +
 ...
 ggtitle('Un-Penalized Smooth Estimate for Japanese Cherry Blossom Data') +
 cherry_theme() +
 coord_cartesian(xlim=(1750, 2010))
)
```

```{python}
def leading0(x): return '0' + str(x) if x < 10 else str(x)
import pandas as pd
spline_df = pd.DataFrame(yearly_spline, columns=[f'BS_{leading0(i)}' for i in range(12)])
spline_df['year'] = flower_df_clean['year']
spline_df_tall = spline_df.melt(id_vars='year')
```